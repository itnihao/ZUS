...
def SSH(queue,command):
	try:
		...
		if str(stderr.read()) == "":
			print "----------%s----------exec""\033[42m %s sucess :)\033[0m" %(str(ipaddr),command)
		else:
			print "----------%s----------exec""\033[41m %s failed :(\033[0m\n" %(str(ipaddr),command)
		ssh.close()
	except Exception,err:
		print "----------\033[41m %s\033[0m----------" %err
...
def TRANS(queue,source,destdir):
	try:
		...
		print "----------%s----------upload""\033[42m %s sucess :)\033[0m" %(str(ipaddr),source)
		trans.close()
	except Exception,err:
		print "----------\033[41m %s\033[0m----------" %err
